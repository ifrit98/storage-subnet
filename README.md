...
## Docker
For convenience, there is a provided `Dockerfile` and `docker-compose.yml` for running miner, validator, or api nodes.

In order to use this option, you **MUST** build the docker image first and then run the docker-compose. Pass your `wandb` api key as an argument to the build script:
```bash
docker build --build-arg WANDB_API_KEY=<WANDB_API_KEY> -t storage-subnet:latest .
```

Once you have successfully built the docker image, you must properly setup the docker-compose.yml, replacing values in carets <>:
```bash
version: '3.7'

services:
  my_container:
    image: storage-subnet:latest
    container_name: sn21
    network_mode: host
    environment:
      REDIS_HOST: localhost
      REDIS_PASSWORD: # this should be set or left blank if not password protected (NOT RECOMMENDED!)
      PYTHONPATH: "/root/.bittensor:${PYTHONPATH}"
    volumes:
      - <YOUR_HOME_DIR>/.bittensor/wallets:/root/.bittensor/wallets # expand and add ~/ (home)
      - .:/root/storage-subnet
    restart: always
    command: >
      python /root/storage-subnet/neurons/<miner|validator|api>.py # Pick one
      --wallet.name <YOUR_COLDKEY> # coldkey name, e.g. default
      --wallet.hotkey <YOUR_HOTKEY> # hotkey name, e.g. validator
      --netuid 21
      --subtensor.network local # can be finney, or a specific chain_endpoint arugment
      --logging.debug
      ... # add other optional arguments as necessary
   ...
```

Once the docker-compose.yml is properly filled, run the miner, validator, or api using docker-compose:
```bash
docker-compose up -d
```
